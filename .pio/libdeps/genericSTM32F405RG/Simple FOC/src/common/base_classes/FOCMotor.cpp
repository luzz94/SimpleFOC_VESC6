...
/** Custom feature IAS lab LB - march 2022
	Sensor linking method
*/
void FOCMotor::linkVelSensor(Sensor* _sensor_v) {
  sensor_v = _sensor_v;
}
...
/** Custom feature IAS lab LB - march 2022
	Sensor linking method
*/
void FOCMotor::linkPosSensor(Sensor* _sensor_p) {
  sensor_p = _sensor_p;
}
...
float FOCMotor::shaftAngle() {
  ...
  */
  ...
  if(!sensor && !sensor_p) return shaft_angle;
  if(sensor) return sensor_direction*LPF_angle(sensor->getAngle()) - sensor_offset;
  else return sensor_direction*LPF_angle(sensor_p->getAngle()) - sensor_offset;
}
...
float FOCMotor::shaftVelocity() {
  ...
  */
  ...
  // if no sensor linked return previous value ( for open loop )
  if(!sensor && !sensor_v) return shaft_velocity;
  if(sensor) return sensor_direction*LPF_velocity(sensor->getVelocity());
  else return sensor_direction*LPF_velocity(sensor_v->getVelocity());
}
...
float FOCMotor::electricalAngle(){
  ...
  */
  ...
  // if no sensor linked return previous value ( for open loop )
  if(!sensor && !sensor_v) return electrical_angle;
  if(sensor) return _normalizeAngle( (float)(sensor_direction * pole_pairs) * sensor->getMechanicalAngle()  - zero_electric_angle );
  else return _normalizeAngle( (float)(sensor_direction * pole_pairs) * sensor_v->getMechanicalAngle()  - zero_electric_angle );
}
