...
int BLDCMotor::alignSensor() {
  ...
  /**Custom feature IAS lab LB - june 2022
  */
  ...
  if(sensor && !sensor_v){
    ...
    if(sensor_v->needsSearch()) exit_flag = absoluteZeroSearch();
  }
  ...
  if(!_isset(sensor_direction)){
   ...
   // take and angle in the middle
    ...
    if (sensor_v) mid_angle = sensor_v->getAngle();
    else mid_angle = sensor->getAngle();
    ...
    if (sensor_v) end_angle = sensor_v->getAngle();
    else end_angle = sensor->getAngle();
  ...
  if(!_isset(zero_electric_angle)){
    ...
    _delay(700);
    // read the sensor
    if (!sensor_v) sensor->update();
    else sensor_v->update();
...
int BLDCMotor::absoluteZeroSearch() {
  ...
  if(!sensor_v){
    ...
    while(sensor_v->needsSearch() && shaft_angle < _2PI){
      angleOpenloop(1.5f*_2PI);
      // call important for some sensors not to loose count
      // not needed for the search
      sensor_v->update();
  ...
  if(monitor_port){
    if(!sensor_v){
      ...
      if(sensor_v->needsSearch()) SIMPLEFOC_DEBUG("MOT: Error: Not found!");
      else SIMPLEFOC_DEBUG("MOT: Success!");
    }
  }
  if(!sensor_v) return !sensor->needsSearch();
  else return !sensor_v->needsSearch();
}
...
void BLDCMotor::loopFOC() {
  ...
  else if (sensor_v) sensor_v->update();
  // to verify
  if (sensor_p) sensor_p->update();
